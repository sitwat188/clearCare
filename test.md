...
**Role Description:**
Specialized role for monitoring HIPAA compliance, conducting audits, and managing security incidents. Read-only access to audit logs and compliance data.
...
**OAuth Scopes:**
- `openid`, `profile`, `email`
- `compliance:audit` - View all audit logs (read-only)
- `compliance:reports` - Generate compliance reports
- `compliance:incidents` - View security incidents
- `compliance:metrics` - View compliance metrics

---

### 5. System Auditor (Optional)

**Role Description:**
External auditors or internal audit team members who need temporary, time-limited access to audit logs for compliance reviews.

**Permissions:**
- ✅ View audit logs (read-only, time-limited)
- ✅ Export audit logs (with restrictions)
- ✅ Generate compliance reports
- ❌ All write operations restricted
- ❌ Access automatically expires after audit period

**OAuth Scopes:**
- `openid`, `profile`
- `audit:read` - Read audit logs (time-limited)
- `audit:export` - Export audit logs (with approval)
...
2. Administrator views existing roles:
   - Default roles (Patient, Provider, Administrator, Compliance Officer)
   - Custom roles
...
#### UC-C-001: Conduct Compliance Audit

**Actor:** Compliance Officer

**Precondition:**
- Compliance Officer is authenticated
- Compliance Officer has audit access permissions

**Main Flow:**
1. Compliance Officer navigates to "Compliance Audit"
2. Compliance Officer selects audit scope:
   - Full system audit
   - Time-period audit (date range)
   - User-specific audit
   - Resource-specific audit (patient, instruction)
   - Incident-based audit
3. Compliance Officer sets audit parameters:
   - Date range
   - Users to include
   - Actions to audit
   - Resources to audit
4. Compliance Officer clicks "Run Audit"
5. System compiles audit data:
   - PHI access patterns
   - Unauthorized access attempts
   - Data modification patterns
   - User activity analysis
   - Policy compliance metrics
6. System generates audit report with:
   - Executive summary
   - Access pattern analysis
   ...
   - Recommendations
   - Detailed findings
7. Compliance Officer reviews audit report
8. Compliance Officer documents findings:
   - Identifies compliance issues
   - Documents policy violations
   - Notes areas of concern
   - Records recommendations
9. Compliance Officer saves audit documentation
10. System logs audit execution in audit trail
...
**Business Rules:**
- Audit reports are read-only for compliance officers
- All audit executions are logged
- Audit reports are retained per retention policy
- Findings can trigger remediation workflows

---

#### UC-C-002: Investigate Security Incident

**Actor:** Compliance Officer

**Precondition:**
- Compliance Officer is authenticated
- Security incident has been flagged or reported

**Main Flow:**
1. Compliance Officer receives security alert or navigates to "Security Incidents"
2. Compliance Officer views incident list:
   - Incident ID and type
   - Severity level
   - Status (new, investigating, resolved)
   - Reported date
3. Compliance Officer selects incident to investigate
4. System displays incident details:
   - Incident type (unauthorized access, data breach, policy violation)
   - Affected users
   - Affected resources (patients, instructions)
   - Timeline of events
   - Initial assessment
5. Compliance Officer reviews related audit logs:
   - Filters audit logs by incident parameters
   - Reviews user activity around incident time
   - Identifies access patterns
   - Reviews related security events
6. Compliance Officer investigates:
   - Reviews user accounts involved
   - Examines data accessed
   - Analyzes access patterns
   - Identifies root cause
7. Compliance Officer documents investigation:
   - Incident timeline
   - Root cause analysis
   - Affected data and users
   - Impact assessment
   - Remediation steps taken
8. Compliance Officer determines if breach occurred:
   - **If breach:**
     - Documents breach details
     - Initiates breach notification workflow
     - Notifies affected individuals (if required)
     - Reports to regulatory authorities (if required)
   - **If no breach:**
     - Documents false positive
     - Closes incident
9. Compliance Officer updates incident status
10. System logs all investigation activities
11. System generates incident report

**Postcondition:**
- Security incident is investigated
- Investigation is documented
- Breach notification initiated (if applicable)

**Business Rules:**
- All investigation activities are logged
- Breach determination follows HIPAA guidelines
- Breach notifications must be timely (within required timeframe)
- Incident reports are immutable once finalized
...
1. **Data Immutability**
   ...
   - Acknowledgment records cannot be modifie